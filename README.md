...
The plots below are from my **Hugging Face Job** run (L4, 80 steps, G=2, SFT base [`sh4shv4t/parlay-sft-1-5b`](https://huggingface.co/sh4shv4t/parlay-sft-1-5b) → [`sh4shv4t/parlay-grpo-1-5b`](https://huggingface.co/sh4shv4t/parlay-grpo-1-5b)). Points are **TRL log means every 5 steps**; raw series lives in [`results/grpo_train_metrics.json`](results/grpo_train_metrics.json). Regenerate figures with `python scripts/plot_grpo_hf_job_curves.py` (or Colab) after a job if you re-run the pipeline.
